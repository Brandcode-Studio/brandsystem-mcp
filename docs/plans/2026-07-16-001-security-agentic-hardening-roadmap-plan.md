...
## 0.12 — Evidence release — SHIPPED 2026-07-17

(All eight items landed: dogfood capture protocol + tooling, holdout commitment protocol, stdio-process test, negative routing + FP-invocation metric, provider adapters, deterministic e2e job scenario, extraction two-lane split (corpus gaps tracked in #35), per-tool schemas ×4, yaml minor merged with majors individually parked. Holdout *content* accrues from dogfood capture — freeze when ~50+ captures across ≥3 sources exist. See CHANGELOG 0.12.0.)
